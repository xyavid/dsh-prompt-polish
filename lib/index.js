...
var Config = z.object({
  enabled: z.boolean().default(DEFAULT_CONFIG.enabled).description("\u603B\u5F00\u5173\uFF1A\u5173\u95ED\u540E\u9690\u85CF\u8F93\u5165\u6846\u65C1\u7684\u5C0F\u661F\u661F\u6309\u94AE\u3002"),
  ...
  strength: z.union([
    ...
    z.const("balanced"),
    z.const("aggressive")
  ]).default(DEFAULT_CONFIG.strength).description("\u4F18\u5316\u529B\u5EA6\uFF1A\u4FDD\u5B88=\u53EA\u7EA0\u9519\u4E0E\u7ED3\u6784\u5316\uFF1B\u5747\u8861=\u8865\u5168\u9690\u542B\u6761\u4EF6\uFF1B\u6FC0\u8FDB=\u91CD\u5199\u6210\u5B8C\u6574\u4EFB\u52A1\u63CF\u8FF0\uFF08\u957F\u5EA6\u4E0A\u9650\u653E\u5BBD\u5230\u7EA6 1500 \u5B57\u7B26\uFF09\u3002"),
  temperature: z.number().min(0).max(1).step(0.05).default(DEFAULT_CONFIG.temperature).description("\u91C7\u6837\u6E29\u5EA6\uFF1A\u8D8A\u4F4E\u8D8A\u5FE0\u5B9E\u539F\u6587\u3002"),
  maxOutputTokens: z.number().step(1).min(128).max(16384).default(DEFAULT_CONFIG.maxOutputTokens).description("\u5355\u6B21\u4F18\u5316\u7684\u8F93\u51FA token \u4E0A\u9650\u3002\u6FC0\u8FDB\u6863 + \u4F1A\u8BDD\u5F00\u542F\u63A8\u7406\u65F6\u9700\u8981\u66F4\u5927\u989D\u5EA6\uFF1B\u88AB\u622A\u65AD\u65F6\u63D2\u4EF6\u62D2\u7EDD\u5199\u56DE\u5E76\u62A5\u9519\u3002"),
  maxInputChars: z.number().step(1).min(100).max(2e5).default(DEFAULT_CONFIG.maxInputChars).description("\u8F93\u5165\u5B57\u6570\u4E0A\u9650\uFF1A\u8D85\u9650\u62D2\u7EDD\u800C\u4E0D\u662F\u622A\u65AD\uFF08\u622A\u65AD\u4F1A\u6539\u53D8\u539F\u610F\uFF09\u3002"),
  timeoutMs: z.number().step(1).min(5e3).max(6e5).default(DEFAULT_CONFIG.timeoutMs).description("\u5355\u6B21\u4F18\u5316\u7684\u8D85\u65F6\u65F6\u95F4\uFF08\u6BEB\u79D2\uFF09\u3002"),
  systemPrompt: z.string().role("textarea").default(DEFAULT_CONFIG.systemPrompt).description("\u81EA\u5B9A\u4E49\u7CFB\u7EDF\u63D0\u793A\u8BCD\uFF1A\u7559\u7A7A\u4F7F\u7528\u5185\u7F6E\u4F18\u5316\u7B56\u7565\u3002"),
  strategyMode: z.union(["replace-default", "extend-default"]).default(DEFAULT_CONFIG.strategyMode).description("\u81EA\u5B9A\u4E49\u63D0\u793A\u8BCD\u7684\u7EC4\u5408\u65B9\u5F0F\uFF1A\u6574\u4F53\u66FF\u6362\u5185\u7F6E\u7B56\u7565\uFF0C\u6216\u8FFD\u52A0\u5728\u5185\u7F6E\u7B56\u7565\u4E4B\u540E\uFF08\u5185\u7F6E\u7684\u786C\u6027\u7EA6\u675F\u7EE7\u7EED\u751F\u6548\uFF09\u3002"),
  undoWindowMs: z.number().step(1).min(0).max(6e5).default(DEFAULT_CONFIG.undoWindowMs).description("\u201C\u64A4\u56DE\u4F18\u5316\u201D\u5165\u53E3\u7684\u4FDD\u7559\u65F6\u957F\uFF08\u6BEB\u79D2\uFF09\u3002")
});
...
function defaultRouteOf(ctx) {
  try {
    return routeOf(ctx.get("settings")?.get("agent-default-model"));
  } catch {
    return void 0;
  }
}
function apply(ctx, config = { ...DEFAULT_CONFIG }) {
  ...
  const readConfig = () => resolveConfig(scope.get());
